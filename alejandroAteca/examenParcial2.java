      ...
      for (int hour = 0; hour <= 23; hour++) {
        ...
        for (int floor = 7; floor >= 1; floor--) {
          ...
          for (int room = 1; room < 8; room++) {
            ...
            } else if (floor == maintenanceFloor && maintenance && room != 8 / 2) {
                System.out.print(WINDOW_UNDER_MAINTENANCE);

            } else {
              boolean lightOn = Math.random() < 0.5;
              boolean windowOpened = Math.random() < 0.5;
              String windowState = (lightOn ? (windowOpened ? WINDOW_OPENED_ON : WINDOW_CLOSED_ON) : (windowOpened ? WINDOW_OPENED_OFF : WINDOW_CLOSED));

              System.out.print(windowState);

              if (windowState.equals(WINDOW_OPENED_ON) || windowState.equals(WINDOW_CLOSED_ON)) {
